            ...
            {blogData.map((blog) => (
              <article key={blog.id}>
                <div className="group w-full border-[1px] border-gray-200 dark:border-custom2 rounded-2xl p-5">
                  <div className="w-full aspect-2/1 rounded-lg overflow-hidden lg:h-[180px]">
                    <Image
                      src={blog.blogPhoto}
                      alt={blog.title}
                      width={1000}
                  ...
                  <div className="mt-5 lg:text-center">
                    <h1 className="custom-cursor text-[18px] font-semibold text-light dark:text-dark inline-block group-hover:text-custom5 dark:group-hover:text-custom5 transition-colors duration-300 ease-in-out cursor-pointer">{blog.title}</h1>
                    <p className="text-base mt-1.5 text-light2 dark:text-dark2 leading-7 mb-5 line-clamp-3">{blog.description}</p>
                    <span className="flex justify-between items-center flex-wrap text-[14px] text-light dark:text-dark">
                      <h3 className=" border-[1px] border-custom5 p-[3px]">{blog.date}</h3>
